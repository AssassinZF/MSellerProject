...
+ (void)compressOriginalImage:(UIImage *)image toSize:(CGSize)size block:(DataBlock)DataBlock
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        @autoreleasepool {
            UIImage * newImage = [UIImage imageByScalingAndCroppingForImage:image size:size];
            NSData * newimageData = [UIImage compressOriginalImage:newImage toMaxDataSizeKBytes:512];
