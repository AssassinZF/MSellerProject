...
- (void)viewDidLoad {
    ...
    self.homeTableView.mj_footer = [MRefreshFooter footerWithRefreshingTarget:self refreshingAction:@selector(requestGetCateListMoreData)];
...
- (void)home_cellSelectItemAtIndex:(NSInteger)index type:(HomeCell_Type)type
{
    if (_homeModel.cateListArray.count > index) {
        ...
        if (type == HomeCell_Type_Share) {
            if (data.goodsdetaillink && data.goodsdetaillink.length > 0) {
                //分享
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MHomeCateListData *data = [_homeModel.cateListArray objectAtIndex:indexPath.row];
    [self pushPageWithName:@"MGoodsDetailViewPage" animation:YES withParams:@{@"spuid":[NSString stringWithFormat:@"%d", data.spuid], @"spucode":data.spucode}];
}
