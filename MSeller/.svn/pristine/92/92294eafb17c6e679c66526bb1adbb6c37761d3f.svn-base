...
typedef enum{
    MSnapshotMeetsEdgeTop,
    MSnapshotMeetsEdgeBottom,
}MSnapshotMeetsEdge;


@interface MDragCellTableView ()
...
/**自动滚动的方向*/
@property (nonatomic, assign) MSnapshotMeetsEdge autoScrollDirection;


@end

@implementation MDragCellTableView
...
- (BOOL)checkIfSnapshotMeetsEdge{
    ...
    if (minY < self.contentOffset.y) {
        _autoScrollDirection = MSnapshotMeetsEdgeTop;
        return YES;
    }
    if (maxY > self.bounds.size.height + self.contentOffset.y) {
        _autoScrollDirection = MSnapshotMeetsEdgeBottom;
        return YES;
    }
    return NO;
}
...
- (void)startAutoScroll{
    ...
    if (_autoScrollDirection == MSnapshotMeetsEdgeTop) {//向下滚动
