...
@interface MBrowserImageView () <UIScrollViewDelegate>
...
@implementation MBrowserImageView
...
{
    [self.showImg removeAllSubviews];
    _mainUrl=url;
    [self resetImgView];
    
    self.imageViewState = MBrowser_ImageView_Loding;
    
    self.indicator = [[UIActivityIndicatorView alloc] init];
    self.indicator.activityIndicatorViewStyle = UIActivityIndicatorViewStyleWhiteLarge;
    self.indicator.center = self.showImg.center;
    [self.showImg addSubview:self.indicator];
    [self.indicator startAnimating];

    [self.showImg m_setImageWithURL:url placeholderImage:placeholder completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        [self.indicator stopAnimating];

        if (error) {
            self.imageViewState = MBrowser_ImageView_Success;
            ...
            self.imageViewState = MBrowser_ImageView_Error;
