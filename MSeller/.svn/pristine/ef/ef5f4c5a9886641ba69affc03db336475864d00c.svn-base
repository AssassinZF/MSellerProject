...
MDragCellTableViewDelegate, MDragCellTableViewDataSource>
{
    ...
    MStoreManageTableViewCell *manageCell;  //店铺管理cell
    MStoreManageActionType manageType;      //店铺管理cell操作类型
}
...
- (void)clickStoreManageActionWithType:(MStoreManageActionType)type AndCell:(MStoreManageTableViewCell *)cell
{
    ...
    switch (type)
    {
        ...
        {
            [self.controller sendMessageID:mRequestEditGoodsStatusTag messageInfo:@{@"actionType":[NSNumber numberWithInteger:MStoreManageRequestTypeCancelRecommend],@"spuId":[NSNumber numberWithInteger:cell.manageData.spuid]}];
        }
            break;
        ...
        {
            [self.controller sendMessageID:mRequestEditGoodsStatusTag messageInfo:@{@"actionType":[NSNumber numberWithInteger:MStoreManageRequestTypeCancelAttention],@"spuId":[NSNumber numberWithInteger:cell.manageData.spuid]}];
        }
            break;
        case MStoreManageActionTypeAAdd:        //关注列表添加按钮
        {
            [self.controller sendMessageID:mRequestEditGoodsStatusTag messageInfo:@{@"actionType":[NSNumber numberWithInteger:MStoreManageRequestTypeAddRecommend],@"spuId":[NSNumber numberWithInteger:cell.manageData.spuid]}];
        }
            break;
...
- (void)update:(id)data msgID:(int)msgid faildCode:(EDataStatusCode)errCode
{
    if (errCode == eDataCodeSuccess)
    {
        ...
        else if (msgid == mRequestEditGoodsStatusTag)
        {
            switch (manageType)
            {
                case MStoreManageActionTypeCCancel:
                {
                    NSIndexPath *cellIndexPath = [self.storeManageTable indexPathForCell:manageCell];
                    //先移除数组_showInfoList中的cell数据
                    [manageModel.choicenessListArray removeObjectAtIndex:cellIndexPath.section];
                    [self.storeManageTable deleteRowsAtIndexPaths:[NSArray arrayWithObject:cellIndexPath] withRowAnimation:UITableViewRowAnimationTop];
                }
                    break;
                case MStoreManageActionTypeADelete:
                {
                    NSIndexPath *cellIndexPath = [self.storeManageTable indexPathForCell:manageCell];
                    //先移除数组_showInfoList中的cell数据
                    [manageModel.attentionListArray removeObjectAtIndex:cellIndexPath.section];
                    [self.storeManageTable deleteRowsAtIndexPaths:[NSArray arrayWithObject:cellIndexPath] withRowAnimation:UITableViewRowAnimationTop];
                }
                    break;
                case MStoreManageActionTypeAAdd:
                {
                    NSIndexPath *cellIndexPath = [self.storeManageTable indexPathForCell:manageCell];
                    //先移除数组_showInfoList中的cell数据
                    [manageModel.attentionListArray removeObjectAtIndex:cellIndexPath.section];
